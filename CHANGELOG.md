...
- Enh: Custom side menu via `pokerDragon\admin\Module::items` (mdmunir).
...
- Chg: Using `VarDumper::export` to save `data` of `pokerDragon\models\AuthItem` (mdmunir).
...
- Chg: Can customize return of `pokerDragon\admin\components\AccessHelper::getAssignedMenu()` with provide a callback to method (mdmunir). 
...
- Chg: Remove `pokerDragon\admin\components\BizRule` (mdmunir).
...
- Chg: Remove `pokerDragon\admin\Module::allowActions`. Set access control directly with `pokerDragon\admin\components\AccessControl` (mdmunir).
- Chg: Change cache strategy (mdmunir).
- Chg: `pokerDragon\admin\components\DbManager` now inherited from `yii\rbac\DbManager` (mdmunir).
